...
namespace duckdb {
  KafkaFileHandle::KafkaFileHandle(FileSystem &fs, string bootstrap_server, string topic, uint8_t flags):FileHandle(fs, bootstrap_server) {

    conf = rd_kafka_conf_new();
    char errstr[256];
    rd_kafka_error_t *error;

    if (rd_kafka_conf_set(conf, "bootstrap.servers", bootstrap_server.c_str(), errstr,
