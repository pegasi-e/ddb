...
static idx_t OnConflictHandling(TableCatalogEntry &table, ClientContext &context,
  ...
  updated_tuples += HandleInsertConflicts<true>(table, context, chunk, data_table);
  // Also check the transaction-local storage+ART so we can detect conflicts within this transaction
  updated_tuples += HandleInsertConflicts<false>(table, context, chunk, data_table);
...
void DataTable::LocalMerge(TableCatalogEntry &table, ClientContext &context, DataChunk &chunk) {
  ...
  OnConflictHandling(table, context, insert_chunk);
  storage.LocalAppend(append_state, table, context, insert_chunk, true);
  storage.FinalizeLocalAppend(append_state);
}

void DataTable::LocalMerge(TableCatalogEntry &table, ClientContext &context, ColumnDataCollection &collection) {
  LocalAppendState append_state;
  auto &storage = table.GetStorage();
  storage.InitializeLocalAppend(append_state, context);
